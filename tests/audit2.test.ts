...
describe("audit: remaining untested paths", () => {
  it("cancel-offer does not touch pilot capacity", () => {
    ...
    const { result } = simnet.callPublicFn("rho-core-v4", "cancel-offer", [Cl.uint(1)], wallet1);
  ...
  it("a non-owner cannot cancel someone else's offer", () => {
    ...
    const { result } = simnet.callPublicFn("rho-core-v4", "cancel-offer", [Cl.uint(1)], wallet2);
  ...
  it("rejects closing a swap before every cycle is settled", () => {
    ...
    simnet.callPublicFn("rho-core-v4", "accept-offer", [Cl.uint(1), Cl.uint(5_000_000)], wallet2);
    const { result } = simnet.callPublicFn("rho-core-v4", "close-swap", [Cl.uint(1)], deployer);
    expect(result).toBeErr(Cl.uint(109)); // ERR-ALL-CYCLES-NOT-SETTLED
  });

  it("rejects zero notional and zero collateral", () => {
    mint(wallet1);
    const zeroNotional = simnet.callPublicFn("rho-core-v4", "post-offer",
    ...
    const zeroCollateral = simnet.callPublicFn("rho-core-v4", "post-offer",
  ...
  it("rejects operations on a swap or offer that does not exist", () => {
    const noOffer = simnet.callPublicFn("rho-core-v4", "accept-offer",
      [Cl.uint(999), Cl.uint(1_000_000)], wallet2).result;
    expect(noOffer).toBeErr(Cl.uint(101)); // ERR-OFFER-NOT-FOUND

    const noSwap = simnet.callPublicFn("rho-core-v4", "close-swap", [Cl.uint(999)], deployer).result;
  ...
  it("a liquidated swap cannot then be closed", () => {
    ...
    simnet.callPublicFn("rho-core-v4", "accept-offer", [Cl.uint(1), Cl.uint(700_000)], wallet2);
    simnet.callPublicFn("pox-rate-oracle-v2", "submit-cycle-rate",
...
describe("audit: cycle numbering", () => {
  it("reads the cycle from the PoX-5 boot contract, not a derived formula", () => {
    const fromPox = simnet.callReadOnlyFn(
      "ST000000000000000000002AMW42H.pox-5", "current-pox-reward-cycle", [], deployer).result;
    const fromRho = simnet.callReadOnlyFn(
      "rho-core-v4", "get-current-pox-cycle", [], deployer).result;
    // Must agree exactly. The old (/ burn-block-height u2100) derivation did not.
    expect(fromRho).toStrictEqual(fromPox);
  });
});
