...
describe("PoX-5 rate oracle", () => {
  it("derives the Tranche 2 rate from the PoX-5 waterfall", () => {
    ...
    // rate = 680 x 1e12 / 1e6 = 680,000,000 sats per 1M STX
    ...
    expect(result).toBeOk(Cl.uint(680_000_000));
  });
  ...
  it("subtracts the Tranche 1 obligation before Tranche 2's share", () => {
    ...
    expect(result).toBeOk(Cl.uint(850_000_000));
  });
  ...
  it("rejects a second submission for the same cycle", () => {
    ...
    const { result } = simnet.callPublicFn("pox-rate-oracle-v2", "submit-cycle-rate",
  ...
  it("stores the derived pool alongside the raw inputs for auditability", () => {
    ...
    const stored = simnet.callReadOnlyFn(
      "pox-rate-oracle-v2", "get-cycle-rate", [Cl.uint(11)], deployer
    ).result;
    ...
    expect(fields["rate-sats-per-mstx"]).toBeUint(680_000_000);
  });
});
